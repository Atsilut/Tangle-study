...
#
# MSA cutover: edge traffic goes to tangle-study-gateway (not the removed monolith api).

ACA_GATEWAY_HOST="${ACA_GATEWAY_HOST:-tangle-study-gateway}"
ACA_PROMETHEUS_HOST="${ACA_PROMETHEUS_HOST:-tangle-study-prometheus}"
ACA_REDIS_HOST="${ACA_REDIS_HOST:-tangle-study-redis}"

ACA_GATEWAY_HTTP_URL="${ACA_GATEWAY_HTTP_URL:-http://${ACA_GATEWAY_HOST}}"
...
# Back-compat aliases for older scripts that still reference ACA_API_*.
ACA_API_HOST="${ACA_API_HOST:-$ACA_GATEWAY_HOST}"
ACA_API_HTTP_URL="${ACA_API_HTTP_URL:-$ACA_GATEWAY_HTTP_URL}"
