...
public sealed class ChatHubRealtimeIntegrationTests(PostgresTestcontainerFixture postgres, RedisTestcontainerFixture redis)
    : ChatIntegrationTestBase(postgres, redis)
{
    ...
    {
        var user1 = CreateUserForTest(prefix, 1);
        var user2 = CreateUserForTest(prefix, 2);
    ...
    private async Task<long> CreateDirectRoomIdAsync(TestUser user1, TestUser user2)
    {
        AcceptFriendship(user1, user2);
        return (await GetOrCreateDirectRoomAsync(user1, user2.Id)).Id;
    }

    private async Task<long> CreatePlatformGroupRoomIdAsync(TestUser owner, TestUser member)
    {
        var (_, _, groupId) = CreateGroupWithMember(owner, member);
        return (await CreatePlatformGroupChatRoomAsync(owner, groupId, [member.Id])).Id;
    }
}
