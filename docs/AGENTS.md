...
**Groups** live in [`services/Group/`](../services/Group/GROUP.md) (MSA step 5). Other services call group-service via `IGroupClient` for user-delete detach.
...
| Batch media by FK | `IMediaClient.GetMediaByCommentIdsAsync` (media-service: [`services/Media/`](../services/Media/)) |
...
| M-1 | Media | `IMediaClient.GetMediaByPostIdsAsync` (implemented in [`services/Media/`](../services/Media/)) | Post list + group delete | fixed |
...
|-------|--------|-------------|
| **1 — Post media** | M-1, P-1, P-2 | Batch via `IMediaClient.GetMediaByPostIdsAsync` / `DeleteBlobStorageForPostsAsync`; media persistence in [`services/Media/`](../services/Media/). |
